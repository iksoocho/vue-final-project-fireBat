<template>
  <div class="container">
    <h3 style="text-align: center; margin-bottom: 50px; color: #dc3545">
      Sign-up
    </h3>
    <div class="containerOne">
      ...
      <div class="containerThree">
        ...
        <label style="margin-bottom: 0px"
          ><a href="#" style="text-decoration: none">내용보기</a></label
        >
        <hr />
        <input
          type="checkbox"
      ...
      <div class="containerFour">
        <label
          for="email"
          style="margin-bottom: 5px; font-weight: bold; color: black"
          >이메일 인증</label
        ><br />
        <input
          type="email"
...
export default {
  ...
  methods: {
    async sendVerificationCode() {
      ...
      if (!this.canSendVerificationCode) {
        ...
        alert("페스오더 이용약관에 동의 후 가입을 진행해 주세요.");
        return;
      }
      ...
      try {
        const checkResponse = await axios.get(
          `/api/user/email/${encodeURIComponent(this.user_email)}`
        );
  ...
  computed: {
    ...
    canSendVerificationCode() {
      // 필수 약관 체크 여부 확인
      const requiredAgreementsChecked = this.agreements
        .slice(0, 2)
